...
def generate_targets(skymap_path, detection_time=None):
    ...
    glade_table = ascii.read(catalog_path, format='csv',
                             guess=False)
    import healpy as hp
    ...
    for obs in observatories:
        ...
        circum = 90. * u.degree - abs(glade_visible['Dec'] * u.degree) \
        ...
        if contains_dist_estimation:
            probs = (aligo_banana[ipix] * distnorm[ipix] *
                norm(distmu[ipix], distsigma[ipix]).pdf(dists) * dists ** 2)
        else:
